...
void houghTransform(Matrix *image, Matrix *accumulator, char out_type){
 ...
 accumulator->rows = 180;
    accumulator->cols = ceilf(2*(sqrtf(image->rows*image->rows + image->cols*image->cols))) - 1;
 ...
 for(j=0; j<image->cols; j++){
  for(i=0; i<image->rows; i++){
   if(image->pM[ (j*image->rows) + i] > (200)){
    for(theta=0; theta<180; theta++){

                    rho = ( (j)*cosf((theta)*3.14159265358979323846/180.0) ) + ( (i)*sinf(theta*3.14159265358979323846/180.0) );

                    if(out_type=='b'){

                        accumulator->pM[ (int)((ceilf(rho + accumulator->cols/2) * 180.0)) + 180-theta-1] = accumulator->grayscale;
                    }
                    else{

                        accumulator->pM[ (int)((ceilf(rho + accumulator->cols/2) * 180.0)) + 180-theta-1]++;
                    }
    }
   }
  }
